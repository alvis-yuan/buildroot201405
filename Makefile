...
SUNMI_OS_PLATFORM = $(call qstrip,$(BR2_SUNMI_OS_PLATFORM))
export SUNMI_OS_PLATFORM
...
ifeq ($(BR2_ROOTFS_SKELETON_CUSTOM),y)
	tar -zxf $(BR2_ROOTFS_SKELETON_CUSTOM_PATH) -C $(O)
TARGET_SKELETON = $(O)/rootfs
endif
...
ifeq ($(findstring x2600, $(BR2_LINUX_KERNEL_CUSTOM_LOCAL_PATH)) , x2600)
	-mv $(TARGET_DIR)/lib/modules/5.10.186/kernel/drivers/misc/sunmi/TPH/*.ko $(BASE_DIR)/modules
	-mv $(TARGET_DIR)/lib/modules/5.10.186/kernel/drivers/misc/ingenic/tpc_spi/device/*.ko $(BASE_DIR)/modules
	-mv $(TARGET_DIR)/lib/modules/5.10.186/kernel/drivers/misc/ingenic/rmem_manager/rmem_manager.ko $(BASE_DIR)/modules
	-mv $(TARGET_DIR)/lib/modules/5.10.186/kernel/drivers/misc/ingenic/camera_driver/soc_camera.ko $(BASE_DIR)/modules
	-mv $(TARGET_DIR)/lib/modules/5.10.186/kernel/drivers/misc/ingenic/camera_device/*.ko $(BASE_DIR)/modules
else ifeq ($(findstring x1600, $(BR2_LINUX_KERNEL_CUSTOM_LOCAL_PATH)) , x1600)
	-mv $(TARGET_DIR)/lib/modules/4.4.94/kernel/drivers/misc/sunmi/TPH/*.ko $(BASE_DIR)/modules
else
	-mv $(TARGET_DIR)/lib/modules/3.10.14/kernel/drivers/misc/sunmi/TPH/*.ko $(BASE_DIR)/modules
endif
...
target-post-image: $(TARGETS_ROOTFS) target-finalize
	@$(foreach s, $(call qstrip,$(BR2_ROOTFS_POST_IMAGE_SCRIPT)), \
		...
		export PROJECT_MODEL_NAME=$(BR2_PROJECT_MODEL_NAME); \
		OUTDIR=${O} \
...
# printvars prints the values of variables specified in VARS from command line currently defined in our Makefiles
printvars:
	@$(foreach V, \
