...
#define SUNMI_RESERVE_RMEM_KB   2732   //2732KB = 2700KB(camera at begin)+32KB(hwinfo at end)

static char *add_mem_KB(char *str, char *tag,
    unsigned int start, unsigned int size)
{
    if (start != 0)
        str += string_copy(str, " ", 1);
    ...
    str += int_to_string(str, start*1024, 16);

    return str;
}

static char* process_mem_bootargs_80cc(char *cmdargs, int ram_size)
{
    char *args_mem = NULL;
    char *args_mem_end = NULL;
    unsigned int mem_start;
    ...
    ram_size = (ram_size - CONFIG_NMEM_MB - CONFIG_RMEM_MB - CONFIG_RTOS_SIZE_MB)*1024 - SUNMI_RESERVE_RMEM_KB;    //32KB for hwinfo

    /* mem=xxxM@0x0*/
    args_mem = add_mem_KB(args_mem, "mem=", 0, ram_size);

    if (CONFIG_RMEM_MB)
        args_mem = add_mem_KB(args_mem, "rmem=", ram_size, CONFIG_RMEM_MB*1024 + SUNMI_RESERVE_RMEM_KB);
    else
        args_mem = add_mem_KB(args_mem, "rmem=", ram_size, SUNMI_RESERVE_RMEM_KB); //32KB for hwinfo

    if (CONFIG_NMEM_MB)
        args_mem = add_mem_KB(args_mem, "nmem=", ram_size+CONFIG_RMEM_MB*1024+SUNMI_RESERVE_RMEM_KB, CONFIG_NMEM_MB*1024);

    if (CONFIG_RTOS_SIZE_MB)
        args_mem = add_mem_KB(args_mem, "rtos_size=", ram_size+(CONFIG_NMEM_MB+CONFIG_RMEM_MB)*1024+SUNMI_RESERVE_RMEM_KB, CONFIG_RTOS_SIZE_MB*1024);
...
#ifdef CONFIG_SUNMI_HWINFO
	return process_mem_bootargs_80cc(cmdargs, ram_size);
#else
